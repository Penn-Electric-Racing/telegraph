<template>
	<div
		:class="{ tabHeader: true, active: active }"
		ref="tabHeader"
...
export default {
	...
	methods: {
		...
		selectTab(id) {
			...
			this.$store.dispatch("tabs/editTesting", currentValue + 1);
		...
		dragMove(event) {
			if (
				event.target.getAttribute("starting-id") == null ||
				event.target.getAttribute("starting-id") == "null"
			)
				event.target.setAttribute("starting-id", this.tab.id);
		...
		dragEnd(event) {
			if (
				this.$refs.tabHeader.getAttribute("starting") == false ||
