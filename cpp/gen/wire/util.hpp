...
namespace wire {
    namespace util {
        ...
        constexpr const uint32_t crc_table[256] = {
        ...
        constexpr uint32_t crc32_block(const uint8_t* p, size_t size) {
            uint32_t crc = ~0U;
            while (size--) {
        ...
        template<size_t Cap, class Ret, class A, class... Args, int... Indexes> 
            Ret apply_helper(const stdext::inplace_function<Ret(A, Args&&...), Cap>& f, A a,
                                index_tuple< Indexes... >, std::tuple<Args...>&& tup) { 
                return f(a, std::forward<Args>(std::get<Indexes>(tup))... ); 
            } 

        template<size_t Cap, class Ret, class A, class ... Args> 
            Ret apply(const stdext::inplace_function<Ret(A, Args&&...), Cap>& f, 
                            A a, std::tuple<Args...>&&  tup) {
