...
export class Node {
	...
	*nodes() {
	...
	static unpack(proto) {
		switch (proto.node) {
			case "group":
				return Group.unpack(proto);
			case "var":
				return Variable.unpack(proto);
			case "action":
				return Action.unpack(proto);
		}
	}
}
...
export class Action extends Node {
	...
	call(value, timeout = 1) {
		return this._ctx ? this._ctx.call(this, value, timeout) : null;
	}
	...
	pack() {
		return {
			action: {
				...
				pretty: this._pretty,
				desc: this._desc,
				argType: this._argType.pack(),
				retType: this._retType.pack(),
			},
		};
	}
	...
	static unpack(proto) {
		...
		return new Action(
			...
			a.pretty,
			a.desc,
			Type.unpack(a.argType),
			Type.unpack(a.retType)
		);
	}
...
export class Type {
	...
	get valid() {
		return this._class != Type.INVALID._class;
	}
	...
	static unpack(proto) {
		switch (proto.type) {
			...
			case 1:
				return Type.NONE;
			case 2:
				return new Type(proto.name, proto.labels, Type.ENUM._class);
			case 3:
				return Type.BOOL;
			case 4:
				return Type.UINT8;
	...
	toString() {
		switch (this._class) {
			...
			case Type.NONE._class:
				return "none";
			case Type.ENUM._class:
				return "enum " + this._name + " [" + this._labels.join(", ") + "]";
			case Type.UINT8._class:
				return "uint8";
			case Type.UINT16._class:
				return "uint16";
